...
import numpy as np
import logging
from ridge.utils import mult_diag, counter
import random
...
def ridge(stim, resp, alpha, mu0=None, cov0=None, singcutoff=1e-10, normalpha=False, logger=ridge_logger):
    ...
    mu0 and cov0 added by Jeff, if you can find the expected mu and covariance structure sigma of the weights, put them in!
    Parameters
    ----------
    ...
    mu0 : array_like, shape (N,)
        prior distribution for weights
    cov0 : array_like, shape(N, N)
        covariance structure for prior
    if mu0 and cov0 are unspecified, simply run ridge regression
    normalpha : boolean
    ...
    for ua in ualphas:
        ...
        ## this part added by jeff
        if cov0 is not None:
            murep=np.repeat(mu0, UR.shape[1], axis=1)
            # bayesian regression
            D = np.linalg.inv(np.diag(S * S) + (ua**2) * reduce(np.dot, [Vh, cov0, Vh.T]))
            awt = np.array(reduce(np.dot, [Vh.T, D, Vh, cov0, murep[:, selvox]]) + reduce(np.dot, [Vh.T, D, np.diag(S), UR[:, selvox]]))
        else:
...
def ridge_corr(Rstim, Pstim, Rresp, Presp, alphas, mu0=None, cov0=None, normalpha=False, corrmin=0.2,
    ...
    if cov0 is not None:
        UR = reduce(np.dot, [Vh, cov0, mu0]) + \
             reduce(np.dot,[np.diag(S), U.T, Rresp])
        VcV = reduce(np.dot, [Vh, cov0, Vh.T])
    else:
    ...
    for na, a in zip(nalphas, alphas):
        ...
        if cov0 is not None:
            D = np.linalg.inv(np.diag(S * S) + (na ** 2) * VcV)
            pred = np.array(reduce(np.dot, [PVh, D, UR]))
        ...
        if use_corr:
            ...
            # Rcorr = (zPresp * zs(pred)).mean(0) # odd that this doesn't always work
            Rcorr = np.multiply(zPresp, zs(pred)).mean(0)
        else:
...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks, mu0=None, cov0=None,
    ...
    wt = ridge(Rstim, Rresp, valphas, mu0=mu0, cov0=cov0, singcutoff=singcutoff, normalpha=normalpha)
